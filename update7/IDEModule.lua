...
--[[
	...
	* Highlight system changes
		[+] Overall now looks more like a highlight system you see in VS code.
		[+] Changed Highlight transparency, was 0.5 now 0.8
		[+] Changed highlight color
		[+] Highlight now displays over text, instead of behind it, so it actually highlights
		[+] Made it so selected text doesn't turn white when selected, it just keeps the syntax highlighting.
		
	* Performance fixes
		[+] Previous highlighting rendered all of the selected white text, so i removed it. This is also listed above in HL sys changes.
]]

local IDEVersion = "7.0"
...
local Settings = {
	Theme = {
		Syntax = {
			...
			BuiltIn = Color3.fromRGB(121, 184, 242),
			...
			FunctionName = Color3.fromRGB(179, 133, 190),
			Bracket = Color3.fromRGB(255, 171, 101)
		},
	}
}
	...
	funcs.Refresh = function(self)
		...
		for row = 1,maxLines do
			...
			if not lineFrame then
				...
				selectionHighlight.BackgroundTransparency = 0.8
				selectionHighlight.ZIndex = 3
